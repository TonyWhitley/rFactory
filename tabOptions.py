...
class Tab:
    def __init__(self, parentFrame):
        ...
        pass
        tkLabel_Options = tk.Label(
            parentFrame, text='Here a set of options for configuring rFactor')
        ...
        ####################################################
        tkFrame_Monitor = tk.LabelFrame(
            parentFrame, text='Monitor', padx=xPadding)
        tkFrame_Monitor.grid(column=1, row=3, sticky='ew')

        self._createVar('Monitor', 'Monitor')
        _tkRadiobuttons['Monitor'] = tk.Radiobutton(
            tkFrame_Monitor,
            text='Monitor',
        ...
        ####################################################
        tkFrame_AIstrength = tk.LabelFrame(
            parentFrame, text='AI', padx=xPadding)
        tkFrame_AIstrength.grid(column=1, row=4, sticky='ew')

        self._createVar('AIstrength', 100)
        tkLabel_AIstrength = tk.Label(
            tkFrame_AIstrength,
            text='Overall AI strength\n(combined with car\nand track factors)')
        ...
        tkLabel_AIaggression = tk.Label(
            tkFrame_AIstrength,
            text='AI aggression (std. rF2)')
        ...
        tkLabel_AIlimiter = tk.Label(
            tkFrame_AIstrength,
            text='AI limiter (std. rF2)')
        ...
        ####################################################
        tkFrame_Co_programs = tk.LabelFrame(
            parentFrame, text='Other programs to run with rF2', padx=xPadding)
        tkFrame_Co_programs.grid(column=3, row=4, sticky='ew')

        self._createVar('CrewChief', False)
        _tkCheckbuttons['CrewChief'] = tk.Checkbutton(
            tkFrame_Co_programs, text='Crew Chief', variable=self.vars['CrewChief'])
        Tooltip(_tkCheckbuttons['CrewChief'],
                text='Spotter and voice control',
                wraplength=wraplength)

        self._createVar('Discord', False)
        _tkCheckbuttons['Discord'] = tk.Checkbutton(
            tkFrame_Co_programs,
            text='Discord (voice)',
        ...
        _tkCheckbuttons['VolumeControl'] = tk.Checkbutton(
            tkFrame_Co_programs, text='VolumeControl', variable=self.vars['VolumeControl'])
        Tooltip(_tkCheckbuttons['VolumeControl'],
                text='Buttons to adjust volume of rF2 and co-programs',
                wraplength=wraplength)

        self._createVar('TeamSpeak', False)
        _tkCheckbuttons['TeamSpeak'] = tk.Checkbutton(
            tkFrame_Co_programs, text='TeamSpeak', variable=self.vars['TeamSpeak'])
        Tooltip(_tkCheckbuttons['TeamSpeak'],
                text='Voice chat between drivers',
                wraplength=wraplength)

        self._createVar('MyPreCommand', False)
        _tkCheckbuttons['MyPreCommand'] = tk.Checkbutton(
            tkFrame_Co_programs,
            text='My command to run before rF2',
            variable=self.vars['MyPreCommand'])

        self._createVar('MyPostCommand', False)
        _tkCheckbuttons['MyPostCommand'] = tk.Checkbutton(
            tkFrame_Co_programs,
