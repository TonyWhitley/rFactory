...
class Multicolumn_Listbox(object):
        ...
        for i in range(0, self._number_of_columns):
            ...
            if sort:
                self.interior.heading(
                    ...
                    command=lambda col=i: self.sort_by(
                        col,
                        descending=False))
            else:
                self.interior.heading(
    ...
    def update_row(self, index, data):
        ...
        if len(data) == len(self._columns):
            ...
            raise ValueError(
                "The multicolumn listbox has only %d columns" %
        ...
        if self._striped_rows:
            for i in range(index, self._number_of_rows):
                self.interior.tag_configure(
                    list_of_items[i + 1], background=self._striped_rows[i % 2])
    ...
    def insert_row(self, data, index=None):
        ...
        if self._striped_rows:
            ...
            self.interior.tag_configure(
                item_ID, background=self._striped_rows[index % 2])

            for i in range(index + 1, self._number_of_rows):
                self.interior.tag_configure(
                    list_of_items[i], background=self._striped_rows[i % 2])

    def column_data(self, index):
        return [self.interior.set(child_ID, index)
    ...
    def sort_by(self, col, descending):
        ...
        self.interior.heading(
            col, command=lambda col=col: self.sort_by(
                col, not descending))
...
if __name__ == '__main__':
    ...
    mc = Multicolumn_Listbox(root,
                             ...
                             command=on_select,
                             cell_anchor="center",
                             right_click_command=on_right_click)
    mc.interior.grid()
